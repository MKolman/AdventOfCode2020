...
pub fn part_two(input: &str) -> String {
	// 8: 42 | 42 8  => 8: 42 +
	let mut hard_input = input.replace("\n8: 42", "\n8: 42 +");
	// 11: 42 31 | 42 11 31  => 11: 42 31 | 42 {2} 31 {2} | 42 {3} 31 {3} | (and so on)
	let mut rule11 = "\n11: 42 31".to_string();
	for n in 2..=5 {
		rule11 += &format!(" | 42 {{{}}} 31 {{{}}}", n, n);
	}
	hard_input = hard_input.replace("\n11: 42 31", &rule11);
	return part_one(&hard_input);
}

#[test]
fn test() {
	let input = crate::common::get_input(19);
	assert_eq!(part_one(&input), "171".to_string());
	assert_eq!(part_two(&input), "369".to_string());
}
